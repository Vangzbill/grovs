  ...
  return (
    <div className="bg-brand-light">
      ...
      <section className="relative min-h-screen flex items-center px-6 lg:px-20 overflow-hidden">
        ...
        {/* Ornamen Aksara Jawa */}
        <div className="absolute top-30 -left-10 text-[200px] font-serif text-brand-blue-100 z-0 rotate-12">ꦒ</div>
        <div className="absolute bottom-20 -right-20 text-[300px] font-serif text-brand-blue-100 z-0 -rotate-12">ꦗ</div>
        <div className="absolute top-20 right-5 text-[250px] font-serif text-brand-blue-100 z-0">ꦱ</div>
        <div className="absolute bottom-10 left-10 text-[150px] font-serif text-brand-blue-100 z-0 rotate-45">ꦗ</div>
        <div className="absolute top-5 right-20 text-[100px] font-serif text-brand-blue-100 z-0 -rotate-45">ꦒ</div>
        <div className="absolute top-5 left-40 text-[100px] font-serif text-brand-blue-100 z-0 -rotate-0">ꦚ</div>
        <div className="absolute bottom-0 left-80 text-[150px] font-serif text-brand-blue-100 z-0 -rotate-45">ꦧ</div>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">
          ...
          <AnimateOnLoad className="w-full flex justify-center" delay={0.3}>
            <Image
              ...
              height={500}
              className="max-w-md lg:max-w-full object-cover rounded-2xl shadow-2xl"
            />
          </AnimateOnLoad>
        </div>
      </section>
      ...
      <section id="fitur" className="py-24 px-6 lg:px-20 bg-white">
        ...
        <div className="mt-16 grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          <FeatureCard icon={<FaBookOpen />} title="Modul Ajar Terstruktur" />
          <FeatureCard icon={<FaLightbulb />} title="Materi Pembelajaran Mendalam" />
          <FeatureCard icon={<FaChalkboardTeacher />} title="Ilustrasi Cerita Inspiratif" />
          <FeatureCard icon={<FaFeatherAlt />} title="Antologi Karya Pilihan" />
        </div>
      </section>
...
const FeatureCard = ({ icon, title }: { icon: React.ReactNode; title: string }) => (
  <Card className="p-1 group bg-gradient-to-br from-brand-blue-200 to-brand-blue-400 shadow-lg" shadow="none">
